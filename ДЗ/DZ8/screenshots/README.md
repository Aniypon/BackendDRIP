...
- Discover: логи сервиса `dz8-app` с полями `traceId`/`spanId`.
- Discover: логи Postgres (`service=postgresql`).

## Grafana (`screenshots/grafana/`)
- Дашборд `DZ8 Observability`, график `Decision Branch RPS`.
- Дашборд `DZ8 Observability`, график `Postgres Transactions Commit` с query:
